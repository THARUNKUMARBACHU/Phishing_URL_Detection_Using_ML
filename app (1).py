...
model=joblib.load('model_svm.sav')
...
def RESULT():
    ...
    print(url)
    p_features = FeatureExtraction(url[0])
    p_feature_values = p_features.getFeaturesList()
    print(len(p_feature_values))
    output = model.predict([p_feature_values])[0]
    if(output==-1):
        result='BE AWARE!! This URL is NOT SAFE '
    else:
        result='This URL is SAFE!!'
    return render_template('index_phishing.html',prediction_text='{}'.format(result))
